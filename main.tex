...
We begin to see some patterns! When we succeedingly append new field operators to the product, by provoking the ordered pattern for shorter products, we only need to flip terms of the form
$$
    \nomo{\phi(x_1)\phi(x_2)\cdots\phi(x_n)}\phi_+(x_{n+1})
$$
because$\nomo{\phi(x_1)\phi(x_2)\cdots\phi(x_n)}\phi_-(x_{n+1})$ is already properly ordered. In doing so, we are commuting $\phi_+(x_{n+1})$ across all the possible permutations of all the possible subsets of $\phi_-(x_i)$. Apparently, this is tedious, but note that for each $\phi_\pm(x_{\sigma_1})\cdots\phi_\pm(x_{\sigma_{n-1}})\phi_-(x_{\sigma_n})\phi_+(x_{n+1})$, if we swap the last two, we get
$$
\phi_\pm(x_{\sigma_1})\cdots\phi_\pm(x_{\sigma_{n-1}})\phi_+(x_{n+1})\phi_-(x_{\sigma_n})+\phi_\pm(x_{\sigma_1})\cdots\phi_\pm(x_{\sigma_{n-1}})D(x_{\sigma_n}-x_{n+1})
$$
For a fixed $\sigma_n$, the factor before $D(x_{\sigma_n}-x_{n+1})$ will iterate over all normal ordered products of $\phi_\pm(x_{\sigma_1})\cdots\phi_\pm(x_{\sigma_{n-1}})$ by construction. Therefore, in the end, we will have
\begin{equation*}
    \sum_{\sigma_i\in \mathcal{P}_n}\left[\phi_\pm(x_{\sigma_1})\cdots\phi_\pm(x_{\sigma_{n-1}})\phi_+(x_{n+1})\phi_-(x_{\sigma_n})+\nomo{\phi(x_{\sigma_1})\cdots\phi(x_{\sigma_{n-1}})}D(x_{\sigma_n}-x_{n+1})\right]
\end{equation*}
